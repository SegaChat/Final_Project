...
def get_creds():
    file_data = None  # Инициализируем переменную по умолчанию
    try:
        ...
        print(f"Содержимое файла: {file_data}")  # Теперь безопасно использовать file_data здесь
    except:
        create_new_token()

    if file_data is None:  # Проверяем, был ли файл успешно прочитан
        logging.error("Не удалось прочитать файл с данными.")
        return None, None  # Возвращаем None, если файл не был прочитан
